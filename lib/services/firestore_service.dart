...
class FirestoreService {
  ...
  /// Save a final CV to the Library
Future<void> saveCVToLibrary(String userId, CVModel cv) async {
  try {
    if (userId.isEmpty) return;
    ...
    await libraryRef.doc(cv.cvId).set({
      ...cv.toMap(),
      'createdAt': FieldValue.serverTimestamp(),
      'updatedAt': FieldValue.serverTimestamp(),
    }, SetOptions(merge: true));

    debugPrint("✅ CV saved to Library: ${cv.cvId}");
  } catch (e) {
